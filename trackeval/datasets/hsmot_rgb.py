...
class HSMOT_RGB(_BaseDataset):
    ...
    def get_default_dataset_config():
        ...
        default_config = {
            'GT_FOLDER': '/data3/PublicDataset/Custom/HSMOT/test/mot',  # Location of GT data
            'IMG_FOLDER': '/data3/PublicDataset/Custom/HSMOT/test/rgb',  #图像目录
            'TRACKERS_FOLDER': '/data3/litianhao/workdir/hsmot',  # Trackers location
    ...
    def _load_raw_file(self, tracker, seq, is_gt):
        ...
        for t in range(num_timesteps):
            ...
            if time_key in data.keys():
                try:
                    time_data = np.asarray(data[time_key], dtype=np.float)
                except ValueError:
                    if is_gt:
                        raise TrackEvalException(
