...
class CLEAR_OCC(_BaseMetric):
    ...
    def eval_sequence_per_id(self, data, remapped_ids, gt_id_map, seq, cls):
        reverse_gt_id_map = {v: k for k, v in gt_id_map.items()}
        # Initialise results
        res = {}
        for field in self.fields:
            res[field] = 0
        ...
        tp_dict = {fid:0 for fid in remapped_ids}
        fn_dict = {fid:0 for fid in remapped_ids}
        idsw_dict = {fid:0 for fid in remapped_ids}
        ...
        for t, (gt_ids_t, tracker_ids_t) in enumerate(zip(data['gt_ids'], data['tracker_ids'])):
            ...
            matched_dict = dict(zip(matched_gt_ids.tolist(), matched_tracker_ids.tolist()))
            
            for fid in remapped_ids:
                if fid in gt_ids_t:
                    if fid in matched_dict:
                        # tp++
                        tp_dict[fid] += 1
                        # 判断id切换
                        prev_id = prev_tracker_id[fid]
                        current_id = matched_dict[fid]
                        if not np.isnan(prev_id) and prev_id != current_id:
                            idsw_dict[fid] += 1
                    else:
                        # fn++
                        fn_dict[fid] += 1
        ...
        #注释
        # output_csv_path_folder = '/data/users/linxu/code/MOTIP-lth/output_files/msi'
        # output_csv_path = os.path.join(output_csv_path_folder, seq, f"{cls}.csv")

        csv_content = []
        for fid in sorted(tp_dict.keys()):
            csv_content.append([reverse_gt_id_map.get(fid), tp_dict[fid], fn_dict[fid], idsw_dict[fid]])
        ...
        #     writer.writerow(['fid', 'tp', 'fn', 'idsw'])
        #     for fid in sorted(tp_dict.keys()):
        #         writer.writerow([reverse_gt_id_map.get(fid), tp_dict[fid], fn_dict[fid], idsw_dict[fid]])

        # print(f"[INFO] Per-ID stats written to {output_csv_path}")
        return res, csv_content
